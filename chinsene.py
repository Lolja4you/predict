...
history = [14, 10, 9, 5, 12]
new_data = [8, 17, 18, 10, 6]
...
if __name__ == "__main__":
    ...
    if not os.path.exists('db'):
        ...
        # 是否预测正确数据库
        database = {"roll": [data[-2]],
                    'odd_acc_roll': [is_odd == (data[-1] % 2 == 1)],
                    'small_acc_roll': [is_small == (data[-1] < 11)],
                    'real_odd_prev': [],
                    'real_small_prev': [],
                    'p_odd_prev': [],
                    'p_small_prev': [],
        ...
        database['odd_acc_roll'].append(database['real_odd_prev'][-1] == (data[-1] % 2 == 1))
        database['small_acc_roll'].append(database['real_small_prev'][-1] == (data[-1] < 11))

        database['p_odd_acc_roll'].append(database['p_odd_prev'][-1] == (data[-1] % 2 == 1))
        database['p_small_acc_roll'].append(database['p_small_prev'][-1] == (data[-1] < 11))
        database['roll'].append(data[-1])

        print(database['real_small_prev'][-1] == (data[-1] < 11))
        print(database['real_small_prev'][-1])

        print("----------------------------------------------------------")
        print(f"read odd acc: {database['odd_acc_roll'].count(True) / len(database['odd_acc_roll'])}")
        print(f"read small acc: {database['small_acc_roll'].count(True) / len(database['small_acc_roll'])}")
        print(f"predict odd acc: {database['p_odd_acc_roll'].count(True) / len(database['p_odd_acc_roll'])}")
        print(f"predict small acc: {database['p_small_acc_roll'].count(True) / len(database['p_small_acc_roll'])}")

    next_odd, next_small = predict_multi_proc(data)
    print(f"!!!!!!!!!!!last:{new_data} predict start!!!!!!!!!!!!!!!!!!!!!")
    predict_odd = next_odd if database['odd_acc_roll'][-1] else (not next_odd)
    predict_small = next_small if database['small_acc_roll'][-1] else (not next_small)
